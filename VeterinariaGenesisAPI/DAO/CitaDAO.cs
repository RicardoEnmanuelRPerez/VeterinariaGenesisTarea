...
public class CitaDAO : ICitaDAO
{
    ...
    public async Task ActualizarAsync(CitaUpdateDto dto)
    {
        ...
        await using var command = new SqlCommand(Procedimientos.Cita_Actualizar, connection)
        {
            CommandType = CommandType.StoredProcedure
        };

        command.Parameters.Add(new SqlParameter("@ID_Cita", SqlDbType.Int) { Value = dto.ID_Cita });
        command.Parameters.Add(new SqlParameter("@Fecha", SqlDbType.Date) { Value = dto.Fecha.Date });
        command.Parameters.Add(new SqlParameter("@Hora", SqlDbType.Time) { Value = dto.Hora });
        command.Parameters.Add(new SqlParameter("@ID_Mascota", SqlDbType.Int) { Value = dto.ID_Mascota });
        command.Parameters.Add(new SqlParameter("@ID_Veterinario", SqlDbType.Int) { Value = dto.ID_Veterinario });
        command.Parameters.Add(new SqlParameter("@ID_Servicio", SqlDbType.Int) { Value = dto.ID_Servicio });
        command.Parameters.Add(new SqlParameter("@Estado", SqlDbType.VarChar, 50) { Value = dto.Estado });
    ...
    public async Task<List<CitaDto>> ListarCompletadasSinFacturaAsync()
    {
        ...
        await using var command = new SqlCommand(Procedimientos.Cita_ListarCompletadasSinFactura, connection)
        {
            CommandType = CommandType.StoredProcedure
        };
